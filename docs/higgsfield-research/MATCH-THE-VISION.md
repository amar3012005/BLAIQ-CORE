...
3. ✅ **URL/product intake — SHIPPED** (`POST /api/copilot/product-intake`, Admin Intake "paste a product / app link"). Fetches the page (SSRF-guarded), extracts product name / value-props / observed colours+tone, cross-checks vs Brand DNA, returns a ready-to-run brief. Verified live on bundb.de (pulled the real #FF6008 palette + Mensch×Maschine positioning). The Story Writer's brief source.
...
5. ✅ **Brand spokespersons + reference-based — SHIPPED** (ImagePipelinePanel CAST row). ✦ Spokesperson generates a reusable, consistent on-brand presenter (verified live: same person, 4 poses, brand wardrobe); + Reference uploads an external image for "an ad like this, in our brand" (brand-locked recreation). Vera the Director's casting.
6. ✅ **Image-preset apps — SHIPPED** (ImagePipelinePanel APPS row): Billboard · Bullet-time · Relight · Multi-angle · Expand — one-click transforms of the active image via ref, brand-locked. Kano the Cinematographer's lens kit.
7. ✅ **Hooks + virality scoring — SHIPPED** (`POST /api/copilot/hooks`, Studio "Hooks & Virality"). 4 angled 3-sec openers + an honest 0-100 stop-the-scroll score with fixes, brand-tone aware (verified live, scored a concept 38 with a critical verdict). Felix the Editor's QA pass. Premium Seedance/Higgsfield i2v stays key-gated via the existing `ctx.higgsfield` path in `video/pipeline.ts`.
