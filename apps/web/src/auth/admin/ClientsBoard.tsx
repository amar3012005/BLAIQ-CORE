  ...
  return (
    <div style={{ padding: '20px 24px', overflowY: 'auto', height: '100%' }}>
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 12, marginBottom: 16 }}>
        <span style={{ ...sansBold, fontSize: 14, color: PAL.ink }}>Clients</span>
      ...
      {error && <div style={{ ...sans, fontSize: 12, color: '#B91C1C' }}>{error}</div>}
      {!rows && !error && <div style={emptyText}>Loading…</div>}
      {rows && rows.length === 0 && <div style={emptyText}>No clients with jobs yet.</div>}

      {rows && rows.length > 0 && (
        <div style={{ background: PAL.white, border: `1px solid ${PAL.divider}`, borderRadius: 8, overflow: 'hidden' }}>
            ...
            <tbody>
              {rows.map(r => (
                <tr key={r.client}>
                  ...
                  <td style={{ ...cell, textAlign: 'right', color: '#0F6E56' }}>{eur(r.paid)}</td>
                  <td style={{ ...cell, textAlign: 'right', color: r.overdue_amount > 0 ? '#B91C1C' : PAL.muted }}>
