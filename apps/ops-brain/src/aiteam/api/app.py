...
def create_app() -> FastAPI:
    ...
    # L1 input guardrails (added first so it runs outermost)
    from aiteam.api.middleware import InputGuardrailMiddleware
    from aiteam.middleware import TenantMiddleware

    app.add_middleware(InputGuardrailMiddleware)

    # BLAIQ tenant scoping — must wrap every request that hits ops.* tables
    # so the session ContextVar is set before DB access. Added AFTER
    # InputGuardrail so guardrails run first; Starlette executes middleware
