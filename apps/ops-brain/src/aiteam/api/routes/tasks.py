...
class FailureAlchemist:  # removed in BLAIQ unification (Track G); no-op stub
    ...
    async def get_learnings(self, *_args: object, **_kwargs: object) -> list:
        return []


class WhatIfAnalyzer:  # removed in BLAIQ unification (Track G); no-op stub
    def __init__(self, *_args: object, **_kwargs: object) -> None: ...
    async def analyze(self, *_args: object, **_kwargs: object) -> dict:
        return {"status": "disabled", "reason": "what_if_removed"}
